let klyAppsAd = null;
let klyAppsIosWorkAroundEl = null;
let klyAppsIosWorkAroundCounter = 0;
let klyAppsIsBannerLoad = false;
let klyAppsIsInvitationLoad = false;
/**
 * Only set to TRUE if required orientation.
 * @type Boolean
 */
let klyAppsIsOrientationEnable = false;
let klyAppsIsVersionDifferentWarned = false;
let klyAppsPlatform = null;
let klyAppsVersion = '4.7.0';

window.addEventListener('load', klyAppsInitCarolCreative, false);

function klyAppsBindEvents() {
  window.addEventListener('message', klyAppsMessageHandler, false);
}

function klyAppsExpandedLandingCallback(index, isAutomation) {
  if (typeof expandedLandToSlideNumber === 'function') {
    expandedLandToSlideNumber(index, isAutomation);
  }
}

function klyAppsInitCarolCreative() {
  klyAppsSetupCreative();
  klyAppsPlatform = new klyAppsMobilePlatform();
  klyAppsAd = new klyAppsRevolver({
//    autoRotateDirection: 'anti-clockwise', // Default is clockwise.
    expandedLoadedCallback: klyAppsExpandedLoadedCallback,
    expandedLandedCallback: klyAppsExpandedLandingCallback,
    invHiddenCallback: klyAppsPostExpandToProxy,
    invLandedCallback: klyAppsInvLandingCallback,
  });
  klyAppsBindEvents();
  klyAppsPostReadyToProxy();
  klyAppsDisableTouchToParent();
  klyAppsSetIOSWorkaround();
}

function klyAppsDisableTouchToParent() {
...
function klyAppsExpandedLoadedCallback() {
  if (klyAppsIsBannerLoad === true) {
    return;
  }

  klyAppsIsBannerLoad = true;

  if (typeof klyAppsBannerLoaded === 'function') {
    klyAppsBannerLoaded();
  }
}

function klyAppsIosAppendText() {
  klyAppsIosWorkAroundEl.innerHTML += '.';
  klyAppsIosWorkAroundCounter++;

  if (klyAppsIosWorkAroundCounter >= 100) {
    klyAppsIosWorkAroundCounter = 0;
    klyAppsIosWorkAroundEl.innerHTML = '';
  }
}

function klyAppsInvLandingCallback(index, isAutomation) {
  if (typeof invitationLandToSlideNumber === 'function') {
    invitationLandToSlideNumber(index, isAutomation);
  }
}

function klyAppsMessageHandler(event) {
  ...
  if (supportedMsg.version != klyAppsVersion) {
    if (klyAppsIsVersionDifferentWarned === false) {
      console.warn('Proxy and ad version is different! Proxy v' + supportedMsg.version + ' vs Ad v' + klyAppsVersion);
      klyAppsIsVersionDifferentWarned = true;
    }
  }
  ...
  switch (supportedMsg.action) {
    case 'shrink-banner':
      ...
      break;
    case 'resize':
      klyAppsAd.setParentWindowHeight(supportedMsg.data.windowHeight);
      break;
  }
...
function klyAppsPostExpandToProxy() {
  parent.postMessage({owner: 'Innity', adType: 'innity-apps-mobile-carol', action: 'interaction', data: {interaction: 'expand-banner'}, version: klyAppsVersion}, '*');
}

function klyAppsPostReadyToProxy() {
  parent.postMessage({owner: 'Innity', adType: 'innity-apps-mobile-carol', action: 'adReady', data: {orientation: klyAppsIsOrientationEnable}, version: klyAppsVersion}, '*');
}

/**
 * This is a workaround to enable iOS Safari touch events after user scroll the page.
 */
function klyAppsSetIOSWorkaround() {
  if (klyAppsPlatform.getOS() === 'ios') {
    klyAppsIosWorkAroundEl = document.getElementById('innity-apps-ios-workaround');
    setInterval(klyAppsIosAppendText, 500);
  }
}

function klyAppsSetupCreative() {
  if (klyAppsIsInvitationLoad === true) {
    return;
  }

  klyAppsGenerateMainContent(document.getElementById('innity-apps-ad'));
  klyAppsIsInvitationLoad = true;
}

let klyAppsImageSuffix = '_2x';
let klyAppsImagesToLoad = 0;
let klyAppsMaterialGeneratorVersion = '5.0.0';

function klyAppsDimensionRatioImageRename(elementType, materialEl) {
  if (elementType !== 'img') {
    return;
  }
  ...
  materialEl.src = materialEl.src.replace(/.png|.jpg|.jpeg/i, klyAppsImageSuffix + extension);
  klyAppsImagesToLoad++;
  materialEl.addEventListener('load', klyAppsImageLoadCompletedHandler);
}

function klyAppsGenerateMainContent(container, isDimensionRatio = false) {
  klyAppsGetDevicePixelRatio();

  for (let i = 0; i < klyAppsMaterials.length; i++) {
    klyAppsMaterialGenerator(klyAppsMaterials[i], container, isDimensionRatio);
  }

  klyAppsImageLoadDelay();
}

function klyAppsGetDevicePixelRatio() {
  let ratio = 1;
  ...
  if (ratio > 2.0 || document.documentElement.clientWidth >= 768) {
    klyAppsImageSuffix = '_3x';
  }
}

function klyAppsImageLoadCompletedHandler() {
  klyAppsImagesToLoad--;
}

function klyAppsImageLoadDelay() {
  if (klyAppsImagesToLoad > 0)
    setTimeout(klyAppsImageLoadDelay, 100);
  else
  {
    ...
    if (typeof klyAppsBannerLoaded === 'function') {
      setTimeout(klyAppsBannerLoaded, 100);
    }
  }
}
 ...
 */
function klyAppsMaterialGenerator(elementData, container, isDimensionRatio = false) {
  ...
  if (typeof elementData.clickTag !== 'undefined') {
    materialEl.addEventListener('click', function(e) {
      e.stopPropagation();
      klyAppsTriggerClickTag(elementData.clickTag);
    }, false);
  }
  ...
  if (typeof elementData.childs !== 'undefined' && typeof elementData.childs === 'object') {
    for (let i = 0; i < elementData.childs.length; i++) {
      klyAppsMaterialGenerator(elementData.childs[i], materialEl, isDimensionRatio);
    }
  }

  if (isDimensionRatio === true) {
    klyAppsDimensionRatioImageRename(elementType, materialEl);
  }
...
function klyAppsMobileAutoPlayVideo(containerID, videoID, options, videoScreenCapImgSrc, canvasOptions, videouioptions) {
    ...
    function initClass() {
      klyAppsMobileVideoFallbackHelper();
        ...
        if (isNativeVideoSupported() === true) {
            ...
            fallbackPlayer = new klyAppsVideoPreview(containerID, videoID, extraOptions, videoScreenCapImgSrc, canvasOptions, videouioptions);
        }
    };

    function checkCreativeRequirement() {
        if (klyAppsMobileVideoElementGenerator === null) {
            errorLog('generateMaterial or klyAppsMaterialGenerator function didn\'t exist.');
            return false;
        }
        ...
        if (typeof(Browser) !== 'function' && typeof(MobilePlatform) !== 'function' &&
            typeof klyAppsMobilePlatform !== 'function') {
    ...
    function platformDetectorWrapper() {
        ...
        } else if (typeof klyAppsMobilePlatform === 'function') {
          platform = new klyAppsMobilePlatform();
        }
    };
    ...
    function VideoControllerUI(clickCallback, enterFullScreenCallback, toggleSoundCallback) {
        ...
        {id: 'play_btn_when_paused_' + videoID, cssClass: ['status-container'], cssStyle: {display: 'none'}, childs: [
            ...
            for (var i = 0; i < controllerEl.length; i++) {
                klyAppsMobileVideoElementGenerator(controllerEl[i], controlWrapperEl);
            }
        ...
        function generateEndedComponent() {
            ...
            for (var i = 0; i < ctaMaterials.length; i++) {
                klyAppsMobileVideoElementGenerator(ctaMaterials[i], endedWrapperEl);
            }
        };
    ...
    function iosCssWorkaround_() {
      ...
      setTimeout(function() {
        ...
        function clickOnCTA_() {
          ...
          klyAppsTriggerClicktagHandler(extraOptions.urls);
          totalCtaClicked_++;
        }
...
function klyAppsVideoPreview(containerID, videoID, options, videoScreenCapImgSrc, canvasOptions, videouioptions) {
    this.currentTime = 0;
    this.duration = 0;
    ...
    var version = '3.2.2';
        ...
        var videoPlayer;
        var videoUI = null;
        var canvasID = 'klyAppsVideoCanvas-'+videoID;
    ...
    function initClass() {
        if (checkRequirement() === false) {
            ...
            return;
                ...
                videoUI = new klyAppsVideoUI(containerID, canvasID, videoID, extraOptions, videoScreenCapImgSrc, self);
    ...
    function platformDetectorWrapper() {
      if (typeof (Browser) === 'function') {
        ...
        autoplaybrowser = new klyAppsMobilePlatform();
      }
    }
    ...
    function klyAppsVideoUI(containerID, canvasID, videoID, options, videoScreenCapImgSrc, klyAppsVideoPreview) {
        ...
        var customVideoID = '-'+videoID;
                ...
                videoEl = [
                    {id: 'klyAppsLoadContainer'+customVideoID, cssClass:['preview-ui-container'], childs: [
                        {elType:'img', id:'klyAppsLoadBg'+customVideoID, cssClass:['backupimage'], attrs:{'src':videoScreenCapImgSrc}}
                    ]}
                ];
        ...
        function initClass() {
            ...
            for (var i = 0; i < videoEl.length; i++) {
                klyAppsMobileVideoElementGenerator(videoEl[i], document.getElementById(containerID));
            }
            ...
            if(mobileVideo) {
                klyAppsVideoPreview.initVideoPlayer();
            }
            else {
                if (requireHTMLVideo) {
                ...
                if(requireHTMLVideo) {
                    if(mobileVideo){
                        if (options.hideMpgWhenHtmlPlayed === true) {
                                ...
                                klyAppsVideoPreview.pausePreview();
                    ...
                    document.getElementById(videoID).play();
                    document.getElementById('klyAppsLoadContainer'+customVideoID).style.display = 'none';
                    performanceTracker.fireClickToPlay();
                }
        ...
        function videoIsReady() {
            ...
            if(requireHTMLVideo) {
                klyAppsVideoPreview.addListener("play", function(){
                        if(!videoStarted) {
                            toggleHTML5VideoStatus("nosound");
                            playerState.classList.remove("pause-preloader-animation");
                        }
                });
                klyAppsVideoPreview.addListener("pause", function(){
                        if(!videoStarted) {
                            playerState.classList.add("pause-preloader-animation");
                        }
                });
                klyAppsVideoPreview.addListener("ended", function(){
                        if(!videoStarted) {
                            toggleHTML5VideoStatus("previewend");
                        }
                });
...
/**
 * Variable to store the generateMaterial or klyAppsMaterialGenerator
 * function reference.
 * @type function
 */
var klyAppsMobileVideoElementGenerator = null;

/**
 * This function help to convert functions that are different from development
 * version to use same variable.
 */
function klyAppsMobileVideoFallbackHelper() {
  if (typeof generateMaterial === 'function') {
    klyAppsMobileVideoElementGenerator = generateMaterial
  } else if (typeof klyAppsMaterialGenerator === 'function') {
    klyAppsMobileVideoElementGenerator = klyAppsMaterialGenerator;
  }
}
 ...
 */
function klyAppsTriggerClicktagHandler(clicktag) {
  if (typeof triggerClickTag === 'function') {
    triggerClickTag(clicktag);
    return;
  }

  if (typeof klyAppsTriggerClickTag === 'function') {
    klyAppsTriggerClickTag(clicktag);
    return;
  }
...
function klyAppsMobilePlatform() {
  this.debug = debug;
...
function klyAppsRevolver(opts) {
  ...
  function initClass_() {
    if (klyAppsPlatform === null || klyAppsPlatform === undefined) {
      alert('Missing "klyAppsMobilePlatform.js" JS file!');
      return;
    }
  ...
  function shrink() {
    ...
    $(iframe_parent_div).parent().removeClass('expander');

    var elementInnity = document.getElementById("bodyScorllads");


    invContainer_.classList.remove('innity-apps-hide');

    elementInnity.classList.remove("expander");
    ...
    if (typeof parent.klyDpfParam.dfp_use_video_obj == "object") {
  ...
  function rotateLeft1Round_() {
    ...
    rotate_(true);
    /*setTimeout(function(){
      $("#bottom_3d_carousel").addClass("Firstactive");
    }, 500);*/
  }
  ...
  function CarolBanner() {
    ...
    function show() {
      ...
      var iframe_parent_div = window.frameElement ? window.frameElement.parentNode : null;
      console.log(iframe_parent_div);
      $(iframe_parent_div).parent().addClass('expander');

      bannerContainerWithBg_.classList.remove('innity-apps-hide');
      $("#bodyScorllads").addClass('expander');
      $(".remove_innity").addClass("hide");
      $(".unexpand").removeClass("hide");
      resizeHandler_();
...
let klyAppsTimerTrackingID = null;
/**
 * For reference purpose only.
 * @type String
 */
let klyAppsTrackerVersion = '5.0.0';

function klyAppsTrackerReset() {
  klyAppsTriggerTimerStop();
  klyAppsTrackerPauseVideo();
}

function klyAppsTriggerClickTag(clickTag) {
  klyAppsTrackerPauseVideo();
  InnityHTMLAd.click({clickTAG: clickTag});
}

function klyAppsTriggerTimerStart(id) {
  if (klyAppsTimerTrackingID === id) {
    return;
  }

  klyAppsTriggerTimerStop();
  InnityHTMLAd.startTimer(id);
  klyAppsTimerTrackingID = id;
}

function klyAppsTriggerTimerStop() {
  if (klyAppsTimerTrackingID !== null) {
    InnityHTMLAd.stopTimer(klyAppsTimerTrackingID);
    klyAppsTimerTrackingID = null;
  }
}

function klyAppsTriggerTrack(track) {
  InnityHTMLAd.track(track);
}

function klyAppsTrackerPauseVideo() {
  if (typeof klyAppsPauseAllVideo === 'function') {
    // Helper function to pause all video.
    klyAppsPauseAllVideo();
  }
}
